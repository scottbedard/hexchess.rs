...
mod tests {
    ...
    fn test_apply_sequence() {
        ...
        assert_eq!(output2, Err("invalid board: whoops".to_string()));
    ...
    fn test_get_targets() {
        ...
        assert_eq!(output2, Err("invalid position: whoops".to_string()));
        assert_eq!(output3, Err("invalid board: whoops".to_string()));
    }
    ...
    fn test_all_targets() {
        let app1 = App {
            command: Command::AllTargets {
                fen: "1/3/5/7/9/11/5P5/11/11/11/11 w - 0 1".to_string(),
            }
        };
        ...
        assert_eq!(Ok("f5f6,f5f7".to_string()), output1);
        assert_eq!(Err("invalid board: whoops".to_string()), output2);
    }
}